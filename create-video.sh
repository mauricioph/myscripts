...
ffmpeg -i "${input}" -loop 1 -i 011-6pm.png -filter_complex "[0:a]showwaves=s=1920x180:mode=cline,colorkey=0x000000:0.01:0.1,format=yuva420p[v];[1:v][v]overlay[outv]" -map "[outv]" -pix_fmt yuv420p -s1920x1080 -threads 0 -map 0:a -c:v libx264 -maxrate 4M -bufsize 7M -preset ultrafast -profile:v:0 high -level:v:0 4.1 -crf 23 -x264opts:0 subme=0:me_range=4:rc_lookahead=10:me=dia:no_chroma_me:8x8dct=0:partitions=none -g:v:0 72 -keyint_min:v:0 72 -sc_threshold:v:0 0 -c:a aac -b:a 128k -map 0:a -shortest -y "${output}"
